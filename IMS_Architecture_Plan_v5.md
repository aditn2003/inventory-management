...
4. `SET LOCAL ROLE ims_app` — switch the transaction to the non-owner `ims_app` role (subject to RLS)
5. `set_config('app.current_tenant_id', '<uuid>', true)` — pin the tenant for the RLS policy
6. RLS filters all subsequent queries automatically; role and GUC revert on transaction commit/rollback

> **Implementation note:** Steps 4–6 are performed inside the `get_tenant_id` FastAPI dependency (`app/auth/dependencies.py`) via the `set_rls_context()` helper (`app/database.py`). The `ims_app` role is created by Alembic migration `0010`. Seed and migrations run as the table-owner `ims_user` (bypasses RLS via `NO FORCE`).
...
- `ims_app` role: SELECT/INSERT/UPDATE/DELETE on tenant tables, no DDL — activated per-transaction via `SET LOCAL ROLE`
- `ims_user` (table owner): used by Alembic migrations and seed, bypasses RLS (`NO FORCE`)
