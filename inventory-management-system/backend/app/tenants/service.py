...
class TenantService:
    ...
    async def list_tenants(
        ...
        return {
            ...
            "summary": {"total": total_count, "active": active_count, "inactive": inactive_count},
        }
    ...
    async def create_tenant(self, name: str, status: str = "active") -> Tenant:
        ...
        try:
            tenant = await self.repo.create(name=name, status=status, display_id=display_id)
    ...
    async def update_tenant(self, tenant_id: UUID, name: Optional[str], status: Optional[str]) -> Tenant:
